...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		...
		<br>
		<?php if($did){ ?>
		<div class="alert alert-warning"><center>保护文件已生效,如需解除保护请删除 <font color=red> 根目录/api/list_api.lock</font></center></div>
		<?php }else{?>
		<hr>
		<center><font color=red><h4>请稍等,系统正在处理中!</h4></font></center>
		<meta http-equiv="refresh" content="3; url=list_api.php?do=1">
		<?php }?>
	</div>
</div>
<?php }elseif($do=='1'){?>
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	...
	$sql=file_get_contents("https://api.ukyun.cn/website/list/lists.sql");
...
if($e==0) {
	echo '<div class="alert alert-success">导入成功！<br/>SQL成功'.$t.'句/失败'.$e.'句</div><center><font color=red><h4>请稍等,系统正在处理中!</h4></font></center>
		<meta http-equiv="refresh" content="3; url=list_api.php?do=2">';
} else {
...
<div class="panel-body">
<?php
@file_put_contents("list_api.lock", '防止被其他人恶意使用');
echo '<div class="alert alert-success"><font color="#FF0033"><h4>注意!!!</h4><br>如果你的空间不支持本地文件读写，请自行在api目录建立 list_api.lock 文件！(没有此文件所有人都可以导入云端数据库)</font></div>';
}
?>
</body>
</html>