...
if(isset($_POST['user']) && isset($_POST['pass'])){
	...
	$row = $DB->get_row("SELECT * FROM ukyun_user WHERE user='$user' limit 1");
	if($user==NULL or $pass==NULL){
       ...
       exit ("<script language='javascript'>alert('登录失败，验证码错误！');history.go(-1)</script>");
		...
		exit("<script language='javascript'>alert('亲爱的管理员，欢迎回家！');window.location.href='./';</script>");
}elseif(isset($_GET['logout'])){
	setcookie("ukyun_token", "", time() - 604800);
	@header('Content-Type: text/html; charset=UTF-8');
