...
<div class="block-title"><h3 class="panel-title">卡密生成</h3></div>
<div class="card-body">
<p>成功生成以下卡密：</p>';
for ($i = 0; $i < $num; $i++) {
	$km=getkm(18);
	$sql=$DB->query("insert into `ukyun_kms` (`tid`,`km`,`value`,`addtime`) values ('".$tid."','".$km."','".$value."','".$date."')");
	if($sql) {
		echo "<li class='list-group-item'>$km</li>";
	}
}

echo '<br><a href="./kmlist.php" class="btn btn-default btn-block">>>返回卡密列表</a>';
}

elseif($my=='del'){
echo '<div class="block">
<div class="block-title"><h3 class="panel-title">卡密删除</h3></div>
<div class="card-body">';
$id=$_GET['id'];
$sql=$DB->query("DELETE FROM ukyun_kms WHERE kid='$id'");
if($sql){echo '删除成功！';}
else{echo '删除失败！';}
...
elseif($my=='qk'){//清空卡密
echo '<div class="block">
<div class="block-title"><h3 class="panel-title">卡密清空</h3></div>
<div class="card-body">
您确认要清空所有卡密吗？清空后无法恢复！<br><a href="./kmlist.php?my=qk2">确认</a> | <a href="javascript:history.back();">返回</a></div></div>';
}
elseif($my=='qk2'){//清空卡密结果
echo '<div class="block">
<div class="block-title"><h3 class="panel-title">清空卡密结果</h3></div>
<div class="card-body">';
if($DB->query("DELETE FROM ukyun_kms WHERE 1")==true){
...
echo '<div class="block">
<div class="block-title"><h3 class="panel-title">卡密列表</h3></div>
<div class="card-body">
...
if(isset($_GET['kw'])) {
	...
	$numrows=$DB->count("SELECT count(*) from ukyun_kms WHERE{$sql}");
	$con='包含 '.$_GET['kw'].' 的共有 <b>'.$numrows.'</b> 个卡密';
}else{
	$numrows=$DB->count("SELECT count(*) from ukyun_kms WHERE 1");
	$sql=" 1";
...
$rs=$DB->query("SELECT * FROM ukyun_kms WHERE{$sql} order by kid desc limit $offset,$pagesize");
while($res = $DB->fetch($rs))
{
if($res['user']==0) {
	$isuse='<font color="green">未使用</font>';
} else {
	$isuse='<font color="red">已使用</font><br/>使用者QQ:'.$res['user'];
}
echo '<tr><td><b>'.$res['km'].'</b><br/>'.商品名：.$ukyun_func[$res['tid']].'</td><td>'.$isuse.'</td><td>'.$res['addtime'].'</td><td>'.$res['usetime'].'</td><td><a href="./kmlist.php?my=del&id='.$res['kid'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此卡密吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
