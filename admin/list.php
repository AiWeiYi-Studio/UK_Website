...
if(isset($_GET['kw'])) {
	$sql=" `qq`='{$_GET['kw']}'";
	$numrows=$DB->count("SELECT count(*) from ukyun_orders WHERE{$sql}");
	$con='包含 '.$_GET['kw'].' 的共有 <b>'.$numrows.'</b> 个订单';
}else{
	$numrows=$DB->count("SELECT count(*) from ukyun_orders");
	$ondate=$DB->count("select count(*) from ukyun_orders where status=1");
	$sql=" 1";
	$con='系统共有 <b>'.$numrows.'</b> 个订单，其中已完成的有 <b>'.$ondate.'</b> 个。';
}

echo '<div class="block">
<div class="block-title"><h3 class="panel-title">商城订单列表</h3></div>
<div class="card-body">
...
$rs=$DB->query("SELECT * FROM ukyun_orders WHERE{$sql} order by id desc limit $offset,$pagesize");
while($res = $DB->fetch($rs))
{
echo '<tr><td>'.$ukyun_func[$res['tid']].'</td><td>'.$res['qq'].'</td><td>'.$res['addtime'].'</td><td>'.display_zt($res['status']).'</td><td><select onChange="javascript:setStatus(\''.$res['id'].'\',this.value)" class="form-control"><option selected>操作订单</option><option value="0">待处理</option><option value="1">已完成</option><option value="4">删除订单</option></select></td></tr>';
}
?>
          </tbody>
        </table>
